...
count <- read.csv('고용형태별_취업자현황_근무일수.csv',stringsAsFactor=F)
count

count_1 <- melt(count, id=c('년도'), variable.name='고용형태별', value.name='근무일수')
count_1

ggplot(count_1, aes(x=년도, y=근무일수, color=고용형태별)) +
